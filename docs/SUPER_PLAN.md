...
| B | B8 — Year-sweep regression to ≥ 99.7 % (all 8 hours × 5 rubrics) | 🟡 in progress 2026-05-05 — Slices 1+2+3+4 ✅. Slice 5 ✅ `expand_at_redirect` whole-body `@Path` and `@Path:Section` resolver — Sancti/01-05's `[Oratio]` body `@Tempora/Nat1-0` now expands to the actual Christmas-Octave Oratio. **30-day Vespera 1570 sweep: 46.67% → 50.00%** (+3 pts). Cumulative: 26.67% → 50.00% (+23 pts) over slices 3+4+5 | — | next wakeup |
