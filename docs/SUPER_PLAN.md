...
| B | B8 — Year-sweep regression to ≥ 99.7 % (all 8 hours × 5 rubrics) | 🟡 in progress 2026-05-05 — Slices 1+2+3 ✅. Slice 4 ✅ auto-derive `next_day_key` for first-vespers concurrence + `vide Sancti/...` chain handling — `office_sweep` now mirrors what `horas.pl` does internally (Vespera on a feast eve renders the next day's office when it outranks). **30-day Vespera 1570 sweep: 36.67% → 46.67%** (+10 pts; Match 11→14, Differ 13→10). Cumulative: 26.67% → 46.67% (+20 pts) over slices 3+4 | — | next wakeup |
