...
| B | B8 — Year-sweep regression to ≥ 99.7 % (all 8 hours × 5 rubrics) | 🟡 in progress 2026-05-05 — Slices 1-5 ✅. Slice 6 ✅ `first_path_token` strips trailing `;`/`,` so `vide Sancti/12-27;` resolves correctly (was previously looking up the literal `Sancti/12-27;` with semicolon, which never hit). **30-day Vespera 1570 sweep: 50.00% → 56.67%** (+7 pts; St. John octave days). Cumulative: **26.67% → 56.67% (+30 pts)** over slices 3-6 | — | next wakeup |
