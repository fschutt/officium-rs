...
| B | B8 — Year-sweep regression to ≥ 99.7 % (all 8 hours × 5 rubrics) | 🟡 in progress 2026-05-05 — Slices 1-6 ✅. Slice 7 ✅ hyphenated commune subkeys (`vide C6-1`/`C2-1`) + Tempora-feria→Sunday fallback (`Tempora/Epi3-4` inherits Sunday's Oratio; `Tempora/Epi4-0tt` falls to bare `Epi4-0`). **30-day Vespera 1570 sweep: 56.67% → 60.00%** (+3 pts) and crucially **RustBlank dropped to 0** — every cell now produces a body. Cumulative: **26.67% → 60.00% (+33 pts)** over slices 3-7. Remaining 12/30 are pure Differs (mostly Tempora-vs-Sancti precedence — leg-C work) | — | next wakeup |
