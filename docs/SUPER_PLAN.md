...
| B | B8 — Year-sweep regression to ≥ 99.7 % (all 8 hours × 5 rubrics) | 🟡 in progress 2026-05-05 — Slices 1+2 ✅ (`f3faada`, `a93f5e9`). Slice 3 ✅ `parse_vide_targets` extended to handle `ex Sancti/MM-DD` and `@Path` parent-inherit directives — Sancti/01-08 (3rd day Octave of Epiphany) now inherits the Epiphany Oratio via the chain. **30-day Vespera 1570 sweep: 26.67% → 36.67%** (+10 pts; RustBlank 11→6, Match 8→11); 4 new tests | — | next wakeup |
